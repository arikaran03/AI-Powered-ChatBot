...
// src/services/geminiService.js
const BACKEND_API_URL = import.meta.env.VITE_BACKEND_API_URL || "http://localhost:3001/api/gemini";

export async function getEmbeddings(textChunks) {
  console.log("Frontend: Requesting embeddings for chunks via proxy:", textChunks.length);
  try {
    const response = await fetch(`${BACKEND_API_URL}/embed`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({ error: 'Failed to parse error from proxy for embeddings' }));
      console.error("Frontend: Error getting embeddings via proxy:", errorData);
      throw new Error(errorData.error || `Proxy embed request failed: ${response.statusText}`);
    }
    const data = await response.json();
    // console.log("Frontend: Received embeddings data via proxy:", data);
    return textChunks.map((chunk, index) => ({
      chunk,
      embedding: data.embeddings[index] || []
    }));
  } catch (error) {
    console.error("Frontend: Network/other error in getEmbeddings via proxy:", error);
    throw error;
  }
}

export async function getAnswerFromContext(context, question) {
  console.log("Frontend: Requesting answer via proxy for question:", question);
  try {
    const response = await fetch(`${BACKEND_API_URL}/chat`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({ error: 'Failed to parse error from proxy for chat' }));
      console.error("Frontend: Error getting answer via proxy:", errorData);
      throw new Error(errorData.error || `Proxy chat request failed: ${response.statusText}`);
    }
    const data = await response.json();
    // console.log("Frontend: Received answer data via proxy:", data);
    return data.answer;
  } catch (error) {
    console.error("Frontend: Network/other error in getAnswerFromContext via proxy:", error);
    throw error;
  }
}