...
// The worker file should be in your "public" directory and match your pdfjs-dist version
pdfjsLib.GlobalWorkerOptions.workerSrc = `/pdf.worker.min.mjs`; // Corrected to .mjs
...
export async function extractTextFromPdf(file) {
  try {
    ...
    console.error("Error extracting text from PDF:", error); // Log the actual error
    throw new Error("Failed to extract text from PDF."); // Generic message for UI
  }
}
...
export async function extractTextFromImage(file) {
  alert("Client-side OCR is experimental and can be slow. Consider a backend solution.");
  // Placeholder if you implement Tesseract.js
  // try {
  //   const { data: { text } } = await Tesseract.recognize(file, 'eng', { logger: m => console.log(m) });
  //   return text;
  // } catch (error) {
  //   console.error("Error with OCR:", error);
  //   throw new Error("Failed OCR processing.");
  // }
  return Promise.reject("OCR not fully implemented client-side.");
}
